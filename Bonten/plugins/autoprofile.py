...
async def autopicloop():
    ...
    if AUTOPICSTART and Config.DEFAULT_PIC is None:
        if BOTLOG:
            return await Bontenub.send_message(
                BOTLOG_CHATID,
    ...
    while AUTOPICSTART:
        ...
        file = await Bontenub.upload_file(autophoto_path)
        try:
            await Bontenub(functions.photos.UploadProfilePhotoRequest(file))
...
async def custompfploop():
    ...
    while CUSTOMPICSTART:
        ...
        file = await Bontenub.upload_file("donottouch.jpg")
        try:
            if i > 0:
                await Bontenub(
                    functions.photos.DeletePhotosRequest(
                        await Bontenub.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await Bontenub(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await Bontenub(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await Bontenub(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(Config.CHANGE_TIME)
...
Bontenub.loop.create_task(autopfp_start())
Bontenub.loop.create_task(autopicloop())
Bontenub.loop.create_task(digitalpicloop())
Bontenub.loop.create_task(bloom_pfploop())
Bontenub.loop.create_task(autoname_loop())
Bontenub.loop.create_task(autobio_loop())
Bontenub.loop.create_task(custompfploop())
