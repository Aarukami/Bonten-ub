...
async def group_has_sedbot(group):
    if isinstance(group, types.InputPeerChannel):
        full = await Bontenub(functions.channels.GetFullChannelRequest(group))
    elif isinstance(group, types.InputPeerChat):
        full = await Bontenub(functions.messages.GetFullChatRequest(group.chat_id))
    else:
        return False
...
@Bontenub.on(events.NewMessage)
async def on_message(event):
    last_msgs[event.chat_id].appendleft(event.message)


@Bontenub.on(events.MessageEdited)
