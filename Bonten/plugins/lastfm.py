...
async def get_curr_track(lfmbio):  # sourcery no-metrics
    ...
    while LASTFM_.LASTFMCHECK:
        try:
            ...
            user_info = await Bontenub(GetFullUserRequest(LASTFM_.USER_ID))
            ...
            if (
                ...
                try:
                    if BOTLOG and LASTFM_.LastLog:
                        await Bontenub.send_message(
                            BOTLOG_CHATID, f"Attempted to change bio to\n{lfmbio}"
                        )
                    await Bontenub(UpdateProfileRequest(about=lfmbio))
                except AboutTooLongError:
                    short_bio = f"🎧: {LASTFM_.SONG}"
                    await Bontenub(UpdateProfileRequest(about=short_bio))
            if playing is None and user_info.about != DEFAULT_BIO:
                await sleep(6)
                await Bontenub(UpdateProfileRequest(about=DEFAULT_BIO))
                if BOTLOG and LASTFM_.LastLog:
                    await Bontenub.send_message(
            ...
            if BOTLOG and LASTFM_.LastLog:
                await Bontenub.send_message(BOTLOG_CHATID, f"Error changing bio:\n{err}")
        await sleep(2)
