...
from sefgs import Bontenub

from Bonten.Config import Config
from Bonten.helpers.functions import rand_key
from Bonten.helpers.functions.utube import (
    download_button,
    get_yt_video_id,
    get_ytthumb,
    result_formatter,
...
from Bonten.plugins import mention
from Bonten.sql_helper.globals import gvarstatus
from Bonten import CMD_INFO, GRP_INFO, PLG_INFO, check_owner
...
def main_menu():
    ...
    buttons = [
        ...
        (
            ...
            Button.inline(f"Network ({len(GRP_INFO['Network'])})", url = "https://t.me/bonten_community"),  
        ),
        (
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        buttons = [
            (
                ...
                Button.url(
                    "Deploy",
                    "https://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2FMr-confused%2Fcatpack&template=https%3A%2F%2Fgithub.com%2FMr-confused%2Fcatpackhttps://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2FMr-confused%2Fcatpack&template=https%3A%2F%2Fgithub.com%2FMr-confused%2Fcatpack",
                ),
            )
        ]
        markup = event.client.build_reply_markup(buttons)
        ...
        text, msg_entities = await event.client._parse_message_text(
            "First Be a Part of Bonten then use Bonten userbot.", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="𝗕𝗼𝗻𝘁𝗲𝗻 𝗡𝗲𝘁𝘄𝗼𝗿𝗸",
            description="First Be a Part of Bonten then use Bonten userbot",
            url="",
            thumb=photo,
            content=photo,
            send_message=types.InputBotInlineMessageMediaAuto(
...
@Bontenub.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@Bontenub.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@Bontenub.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@Bontenub.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
