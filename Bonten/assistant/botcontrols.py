...
@Bontenub.bot_cmd(pattern="^/help$", from_users=Config.OWNER_ID)
...
@Bontenub.bot_cmd(pattern="^/broadcast$", from_users=Config.OWNER_ID)
...
@Bontenub.bot_cmd(pattern="^/ban\\s+([\\s\\S]*)", from_users=Config.OWNER_ID)
...
@Bontenub.bot_cmd(pattern="^/unban(?:\\s|$)([\\s\\S]*)", from_users=Config.OWNER_ID)
async def ban_botpms(event):
    user_id, reason = await get_user_and_reason(event)
    reply_to = await reply_id(event)
    if not user_id:
